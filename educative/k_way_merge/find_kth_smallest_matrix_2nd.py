...
Time: The Binary Search could take O(log(max - min)) iterations where 'max' is the largest and 'min' is the smallest
...
def count_less_equal(matrix, mid, smaller, larger):
    ...
    while row >= 0 and col < n:
        if matrix[row][col] > mid:
            ...
            # as matrix[row][col] is less than or equal to the mid, let's keep track of the biggest number less than or
            # equal to the mid
...
def main():
    print("Kth smallest number is: " + str(find_kth_smallest([[1, 4], [2, 5]], 2)))
